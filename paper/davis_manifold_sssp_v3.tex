...
algorithm. GIGI (Geometric Intrinsic Global Index), a fiber-bundle database
engine, supplies durable edge records and indexed incidence; its
...
\paragraph{Database context.}
GIGI (Geometric Intrinsic Global Index) is a Rust database engine developed by
the author~\cite{gigi}. Its data model groups keyed records into \emph{bundles}:
base fields identify and index records, while fiber fields hold their associated
values. The fiber-bundle terminology expresses this separation of keys and
values; readers need no differential-geometric background to follow the graph
experiments. The engine integrates record storage and retrieval with statistical
and geometric queries over those records. Here it supplies explicit weighted
edge records, indexed incidence, and a normalized weight-variance feature.
The timed shortest-path solvers run on extracted adjacency lists; the study
evaluates these specific uses rather than the engine's full query language.
Source code and documentation are available at
\url{https://github.com/nurdymuny/gigi}, under the PolyForm Noncommercial
License 1.0.0.
...
\input{figs_v3/core_table.tex}
\input{figs_v3/core_findings.tex}
\begin{figure}[tb]
\centering\includegraphics[width=\linewidth]{figs_v3/core.pdf}
...
\label{fig:core}
\end{figure}
\input{figs_v3/mechanism_table.tex}
\par\begin{samepage}
...
\begin{figure}[tb]
\centering\includegraphics[width=.94\linewidth]{figs_v3/density.pdf}
...
\label{fig:density}
\end{figure}
\input{figs_v3/density_findings.tex}
...
changing any global weight statistic. \input{figs_v3/law_findings.tex}
...
\input{figs_v3/large_table.tex}
\input{figs_v3/large_findings.tex}
...
mapping back. \input{figs_v3/layout_findings.tex}

\subsection{Gates and the cost of speculative work}
\input{figs_v3/gate_table.tex}
\input{figs_v3/gate_findings.tex}
...
credited with free tuning. \input{figs_v3/amortization.tex}
...
\input{figs_v3/policy_findings.tex}
...
The database model permits several different notions of adjacency: shared
indexed values, proximity between numeric record values, or explicitly stored
graph endpoints. The SSSP input in this study uses the last of these. Declaring
base and fiber fields alone does not identify the weighted digraph to be solved.
Separating explicit graph incidence from geometric information has precedent
in the interface design of JDSL/GeomLib~\cite{tamassia-vismara}.
Here, this distinction determines which stored structure defines the
...
distances after snapshot and memory-mapped reload. \input{figs_v3/substrate.tex}
...
applied to the original SSSP graph. \input{figs_v3/k_findings.tex}
...
recorded separately. \input{figs_v3/potential.tex}
...
\bibitem{gigi} B. R. Davis. GIGI: Geometric Intrinsic Global Index.
Rust database engine, source code and documentation, 2026.
\url{https://github.com/nurdymuny/gigi}.
Repository and PolyForm Noncommercial License 1.0.0 inspected September 17, 2026.
\bibitem{tamassia-vismara} R. Tamassia and L. Vismara.
A case study in algorithm engineering for geometric computing.
\emph{International Journal of Computational Geometry \& Applications},
11(1):15--70, 2001. Author preprint:
\url{https://cs.brown.edu/cgc/jdsl/papers/geomlib.pdf}.
\end{thebibliography}
\end{document}
