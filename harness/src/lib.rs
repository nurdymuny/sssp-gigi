//! Library view of the published benchmark, generated from main.rs.
//! Her main.rs is unmodified; this exists only so the ablation binary
//! can call the SAME graph generators and verifier.

#![allow(dead_code)]

pub mod ablation;
pub mod curvature_gate;
pub mod revision;
...
pub struct Graph {
    pub n: usize,
    pub m: usize,
    pub adj: Vec<Vec<(usize, f64)>>,  // adj[u] = [(v, weight), ...]
    pub degree: Vec<usize>,           // Pre-computed out-degrees
}
...
pub fn random_sparse_graph(n: usize, avg_degree: usize, seed: u64) -> Graph {
...
pub fn random_dense_graph(n: usize, density: f64, seed: u64) -> Graph {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
    let mut graph = Graph::new(n);
...
pub fn grid_graph(rows: usize, cols: usize, seed: u64) -> Graph {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
...
pub fn clustered_graph(n_clusters: usize, cluster_size: usize, intra_density: f64, inter_edges: usize, seed: u64) -> Graph {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
...
pub fn road_network_like(n: usize, seed: u64) -> Graph {
    let mut rng = ChaCha8Rng::seed_from_u64(seed);
...
pub fn davis_delta(graph: &Graph, source: usize) -> Vec<f64> {
...
pub fn davis_corridor(graph: &Graph, source: usize) -> Vec<f64> {
    let n = graph.n;
    if n == 0 { return Vec::new(); }
...
pub fn davis_deep(graph: &Graph, source: usize) -> Vec<f64> {
    let n = graph.n;
    if n == 0 { return Vec::new(); }
...
pub fn davis_hub(graph: &Graph, source: usize) -> Vec<f64> {
    let n = graph.n;
    if n == 0 { return Vec::new(); }
...
pub fn verify_distances(d1: &[f64], d2: &[f64]) -> bool {
    if d1.len() != d2.len() {
        return false;
    }
